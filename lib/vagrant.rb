...
def check_providers(allowed_providers)
  allowed_providers.each do |ap|
    if Vagrant.has_plugin?(ap)
      puts "[INFO] Provider #{ap} has been found. Proceeding..."
      provider = :libvirt
      return provider
    end
  end
  puts '[INFO] Provider plugins have not been found...'
  puts '[INFO] Using default virtualbox provider...'
  provider = :virtualbox
  return provider
end
...
def check_plugins(required_plugins)
  ...
  unless plugins_to_install.empty?
    puts '[WARNING] Missing Vagrant plugins...'
    puts '[WARNING] Please install vagrant plugins with the following command:'
    puts "[WARNING] vagrant plugin install #{plugins_to_install.join(" ")}"
    exit 0
  end
end

def generate_ssh_keys(ssh_keys_dir, key_name, comment)
  unless File.file?(File.join(ssh_keys_dir, key_name))
    puts '[INFO] Generate SSH key...'
    system("ssh-keygen -f #{File.join(ssh_keys_dir, key_name)} -q -N '' -t rsa -C '#{comment}'")
